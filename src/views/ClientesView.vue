...
const actualizarEstado = ({ id, estado }) => {
  ClienteService.cambiarEstado(id, { estado: !estado })
    .then(({ data }) => {
      ...
      clientes.value[index].estado = data.estado
    })
    .catch((error) => {
      console.log("🚀 ~ file: ClientesView.vue:32 ~ actualizarEstado ~ error:", error)
    })
}
...
const eliminarCliente = (id) => {
  ClienteService.eliminarCliente(id)
    ...
    .catch((error) => {
      console.log("🚀 ~ file: ClientesView.vue:43 ~ eliminarCliente ~ error:", error)
    })
}
</script>
